...
# • Configure this R file for your dataset & upload to Sherlock
...
is.case.control <- as.logical(input.arguments[3])

# Normalize? 
normalization.enabled <- as.logical(input.arguments[4])

# If fifth argument specified, throw away samples matching the REGEX
if (length(input.arguments) == 5) {
  sample.removal.substring <- input.arguments[5]
}
...
if (exists("sample.removal.substring") && !is.null(sample.removal.substring) && sample.removal.substring != "" && sample.removal.substring != "FALSE") {
