...
# Only do this if you need a specific release.
...
map.gene.names <- function(protein.column, download.chunk.size = 2000, taxonomy.id = 9606) {

  # get a list of all unique UniProt identifiers in the provided protein column
  unique.proteins <- unique(unlist(strsplit(as.character(protein.column), ";")))

  # strip any ptms off UniProt identifiers if exist
  UniProt.IDs <- unique(sapply(unique.proteins, function(x) strsplit(x, "_")[[1]][1]))

  # UniProt REST API can arbitrarily truncate large (6000+) queries
  # Do in smaller chunks to avoid issues
  chunk_size <- download.chunk.size
  id_chunks <- split(UniProt.IDs, ceiling(seq_along(UniProt.IDs) / chunk_size))

  # Chunked API call
  mapping_list <- lapply(id_chunks, function(ids) {
    AnnotationDbi::select(
      UniProt.ws(taxId=taxonomy.id),
      keys = ids,
      columns = c("UniProtKB", "gene_primary"),
      keytype = "UniProtKB"
    )
  })
  UniProt.mapping <- do.call(rbind, mapping_list)

  # Throw warning if # UniProt IDs do not match # mapped gene names
  n_mapped <- nrow(UniProt.mapping)
  n_total <- length(unique(UniProt.IDs))

  if (n_mapped != n_total) {
    missing_ids <- setdiff(unique(UniProt.IDs), UniProt.mapping$UniProtKB)

    warning(sprintf(
      "%d of %d UniProt IDs did not map to gene names (%.1f%%). This should not happen for canonical proteomes.",
      length(missing_ids), n_total, 100 * length(missing_ids) / n_total
    ))

    cat(sprintf("Missing IDs (showing up to 10 of %d):\n", length(missing_ids)))
    print(head(missing_ids, 10))

    if (interactive()) {
      response <- readline(prompt = "Continue anyway? (y/n): ")
      if (!tolower(trimws(response)) %in% c("y", "yes")) {
        stop("Aborted by user due to incomplete UniProt mapping.")
      }
    } else {
      stop(
        "Incomplete UniProt mapping detected in non-interactive session. ",
        "Aborting rather than prompting."
      )
    }
  }

  # Lookup table for quick mapping
  gene.name.map <- setNames(UniProt.mapping$Gene.Names..primary., UniProt.mapping$From)

  # Row-wise, map all UniProt IDs to to gene names while accounting for PTMs
  map_protein_group <- function(entry) {

    # Split the entry by semicolons
    protein.groups <- unlist(strsplit(entry, ";"))

    # Replace UniProt IDs with gene names
    protein_mapping <- sapply(protein.groups, function(protein) {

      # Special code to handle PTMs
      parts <- unlist(strsplit(protein, "_"))
      UniProtID <- parts[1]
    ...
    paste(protein_mapping, collapse = ";")
  }

  # Apply the mapping function to each row of the protein column
  mapped_column <- sapply(as.character(protein.column), map_protein_group)
...
map.gene.names.clusterProfiler <- function(protein.column, protein.database.name = "org.Hs.eg.db") {
  ...
  # load in correct protein database
...
It's recommended to include commonly observed contaminant proteins when processing your proteomic datasets. However, Spectronaut does not automatically filter these entries for you. Fortunately, all contaminant protein accessions are prepended with the text **Cont\_** so we can go through each row in our dataset and toss those data ourselves.
...
### Make Replicate column unique for each experimental condition
...
if (export.type == "SP") {
  ...
  global.prepared <- suppressWarnings({
      SpectronauttoMSstatsFormat(spectronaut.lfq.report,
...
ggsave(paste(paste(data.name, "figures", sep = "_"), "Boxplot - Peptide Transition Intensities.pdf", sep = "/"))

```

### Remove any unwanted runs

You'll frequently want to remove certain samples or conditions from your dataset before summarizing to the protein level. There are many ways of doing this. I usually do this by matching certain text in the **BioReplicate** or **Condition** columns. The below chunk tosses any files listed in a vector named *runs.to.remove*, but you could also use the dplyr filter function.
...
#global.proteinSummarization$ProteinLevelData$gene.name <- map.gene.names.clusterProfiler(global.proteinSummarization$ProteinLevelData$Protein)
...
Filtered.Sites <- as.data.table(
  global.pairwiseComparison %>% 
    group_by(Protein) %>% 
    filter(any(pvalue <= pvalue.threshold & abs(log2FC) >= log2FC.threshold)))

Sig.Changing.Site.Matrix <- as.matrix(dcast(Filtered.Sites, gene.name ~ Label, value.var = "log2FC"), rownames = "gene.name")
...
# Get maximum magnitude Log2FC for heatmap
log2fc.magnitude <- global.pairwiseComparison %>%
  filter(is.finite(log2FC)) %>% 
  summarise(max_abs = max(abs(log2FC), na.rm = TRUE)) %>%
  pull(max_abs)

log2fc.magnitude <- round(log2fc.magnitude)

# Center at 0
heatmap.colorscale <- colorRamp2(
  breaks = seq(-log2fc.magnitude, log2fc.magnitude, by = 1),
  colors = colorRampPalette(c("blue", "#EEEEEE", "red"))(length(seq(-log2fc.magnitude, log2fc.magnitude, by = 1)))
)

heatmap <- Heatmap(
  Sig.Changing.Site.Matrix,
  column_title = "Example Heatmap",
  row_title = sprintf("%d significantly changing phosphosites vs control", nrow(Sig.Changing.Site.Matrix)),
  name = "Log2 fold change from control\n",
  col = heatmap.colorscale,
  show_row_names = FALSE,
  row_names_gp = gpar(fontsize = 8),
  cluster_columns = TRUE,
  heatmap_legend_param = list(
    at = seq(-log2fc.magnitude, log2fc.magnitude, by = 1),
    labels = seq(-log2fc.magnitude, log2fc.magnitude, by = 1)
  )
)
...
grid::grid.text(
  ...
  "Significance defined as pvalue < 0.05 and log2FC > 1",
  y = unit(0.03, "npc"), 
  gp = gpar(fontsize = 8),
  just = "center"
)
...
#global.go.terms <- Load.GO.Terms.From.Bioconductor(database.name = "org.Hs.eg.db", ontology.type = "CC")

# This is all the proteins in your dataset.
dataset.all.proteins <- unique(as.character(global.pairwiseComparison$Protein))

# get indices of clusters of interest based on dendrogram
beads40.proteins <- (global.pairwiseComparison %>%
  filter(pvalue <= 0.05 & abs(log2FC) >= 1 & Label == "0m_40-ctrl_40"))$Protein

beads80.proteins <- (global.pairwiseComparison %>%
  filter(pvalue <= 0.05 & abs(log2FC) >= 1 & Label == "0m_80-ctrl_80"))$Protein

cluster.list <- list(Beads40 = beads40.proteins, Beads80 = beads80.proteins )
...
#source(paste0(drb_utils,"gene_ontology_enrichment.R"))
cluster.enrichment.table <- rbindlist(enrichment.list, idcol = "Cluster")
