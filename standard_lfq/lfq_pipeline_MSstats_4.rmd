---
title: "lfq_pipeline - MSstats 4.0"
...
# File paths to your mass spec data in Spectronaut format
spectronaut.lfq.report <- "./example_data_spectronaut.tsv"
...
spectronaut.lfq.report <- fread(spectronaut.lfq.report, sep = '\t')

# If fread is causing R to crash due to memory, you can try using read.csv instead. Slower, but uses less memory. 
# spectronaut.lfq.report <- read.csv(spectronaut.lfq.report, sep = '\t')
...
### Implement MSstats V4.0 pipeline instead of straight MSstats::dataProcess() function
```{r}

normalization <- "equalizeMedians"
summary.method = "TMP"

intermediate.dataset = MSstatsPrepareForDataProcess(global.prepared, log_base = 2, fix_missing = "zero_to_na")
intermediate.dataset = MSstatsNormalize(intermediate.dataset, normalization)
intermediate.dataset = MSstatsHandleMissing(intermediate.dataset, summary.method, impute = TRUE, missing_symbol = "NA", censored_cutoff = 0.999)
intermediate.dataset = MSstatsSelectFeatures(intermediate.dataset, "highQuality", top_n = 3, min_feature_count = 2)
intermediate.dataset = MSstatsPrepareForSummarization(intermediate.dataset, summary.method, impute = TRUE, censored_symbol = "NA", remove_uninformative_feature_outlier = FALSE)

summarized.dataset = MSstatsSummarizeWithSingleCore(intermediate.dataset, summary.method, impute = TRUE, censored_symbol = "NA", remove50missing = FALSE, equal_variance = TRUE)
# MacOS and Linux can use MSstatsSUmmarizeWithMultiCore
# summarized.dataset = MSstatsSummarizeWithMultiCore(intermediate.dataset, summary.method, impute = TRUE, censored_symbol = "NA", remove50missing = FALSE, equal_variance = TRUE)
global.proteinSummarization = MSstatsSummarizationOutput(intermediate.dataset, summarized.dataset, processed.dataset, summary.method, impute = TRUE, censored_symbol = "NA")
...
global.proteinSummarization$ProteinLevelData <- fread(paste(paste(data.name, "data", sep = "_"), "20250130_ProteinLevelData.csv", sep='/'))
global.proteinSummarization$FeatureLevelData <- fread(paste(paste(data.name, "data", sep = "_"), "20250130_FeatureLevelData.csv", sep='/'))
...
ggsave(paste(paste(data.name, "figures", sep = "_"), "Protein Abundances per Sample.pdf", sep = "/"), width= 15)
...
ggsave(paste(paste(data.name, "figures", sep = "_"), "UniqueProteinGroupsPerCondition.pdf", sep = "/"), width= 15)
```
...
ggsave(paste(paste(data.name, "figures", sep = "_"), "Protein CVs per Condition.pdf", sep = "/"), width= 15)
