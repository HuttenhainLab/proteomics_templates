...
Differs from the standard LFQ pipeline by leveraging the MSstatsBig import function instead of the standard converter
...
The Spectronaut report for large datasets are too large to read into memory for the primary MSstats pipeline. To solve this, we need to use an alternative approach to pre-process the Spectronaut export. To do this, we use a package named MSstatsBig which handles the Spectronaut export in chunks.
```{r}

# Before pre-processing, we need to hot-swap the values of "F.PeakArea" and "F.NormalizedPeakArea" in the input file as MSstatsBig only uses normalized peak area.

# File needs to go in [DATA.NAME]_data/swapped_[INPUTFILE.TSV]
swapped.report <- paste(data.name, "data/swapped", basename(spectronaut.lfq.report), sep = "_")

# Read the column headers of the input files to get the column indices of the columns to swap
fread(spectronaut.lfq.report, nrows = 1) -> header_df  
header <- names(header_df)
column.area <- which(header == "F.PeakArea")
column.normalized.area <- which(header == "F.NormalizedPeakArea")

# Open simultaneous reader and writer
reader <- file(spectronaut.lfq.report, "r")
writer <- file(swapped.report, "w")

# Read and write header
header_line <- readLines(reader, n = 1)
writeLines(header_line, writer)

# Process line by line
repeat {

    line <- readLines(reader, n = 1)
  if (length(line) == 0) break  # Stop if EOF

  values <- strsplit(line, "\t")[[1]]
  
  # Swap the two columns
  values[column.normalized.area] <- values[column.area]
  
  # Write the modified line
  writeLines(paste(values, collapse = "\t"), writer)
}
...
global.prepared <-  bigSpectronauttoMSstatsFormat(
  input_file = swapped.report,
...
global.prepared <- as.data.frame(dplyr::collect(global.prepared))
...
global.prepared <- global.prepared %>%
  filter(!grepl("Cont_", ProteinName))
...
```{r}
if (is.case.control) {
  global.prepared$BioReplicate <- paste(global.prepared$Condition, global.prepared$BioReplicate, sep = ".")
}
```
...
ggplot(global.prepared, aes(x=log10(Intensity))) +
  ...
  labs(title = "Histogram of Peptide Transition Intensities", x = "Log10(Intensity)", y = "Frequencies") +
  theme_bw()
