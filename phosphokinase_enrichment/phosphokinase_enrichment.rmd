...
# Required Parameters
This dataset assumes that you have already run the Huttenhain phosphoproteomic pipeline to get a set of group comparisons. 
```{r, Params}
species <- "HUMAN" # HUMAN MOUSE and RAT are only species with their own kinase substrate data, otherwise, uncomment next line and supply filepath to mapped kinase data
#kinaseMap <- fread("")
...
dataName <- "MG05"
outputDirectory <- "Phospho_kinase_enrichment_data"
...
if (computeKinase == TRUE){
  ...
  # Load kinase Data
  if (!exists("kinaseMap")){
    #kinaseMap <-loadKinaseDataOmniPath()           # OmnipathR import_omnipath_enzub() errors in recent versions, just use locally downloaded kinase map instead
    
    if (species == "HUMAN"){
      kinaseMap <- fread("data/HumanKinaseDataOmniPath.csv")
    }
    if (species == "MOUSE"){
      kinaseMap <- fread("data/MouseKinaseDataOmniPath.csv")
    } 
    
    stopifnot( `species argument must be "MOUSE" or "HUMAN". To perform kinase analysis on another species, import your own kinase map and assign it to kinaseMap variable or map your species' gene/site names to human or mouse genes/sites` = species == "HUMAN" | species == "MOUSE" )
...
plotKinases <- function(reg, seaBool = FALSE, dirAnot = "zScore", name = NULL, scores = kinActFull.scores,mapped = kinActFull.mapped, max_fdr = 0.2, sigKinases = NULL, save = F){
  
  barPlot <- BarplotKinaseActivities(kinActFull.scores[grepl(reg,kinActFull.scores$Label)], kinActFull.mapped[grepl(reg,kinActFull.mapped$Label)], max_pValue = 0.05, max_fdr = max_fdr, useSEA = seaBool, sigKinases=sigKinases)
