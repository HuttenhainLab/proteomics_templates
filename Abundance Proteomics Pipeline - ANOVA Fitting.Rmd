...
data.name <- "EB10_Abundance"

# create subdirectories to store intermediate tables and figures
Create.Pipeline.Directories(data.name)

protein.data.path <- "./EB10_data/AB/20250201_ProteinLevelData.csv"

# name of the labels you will split your dataset on
series.labels <- c("Iso_1min", "Iso_15min", "NECA_1min","NECA_15min", "VIP_1min", "VIP_15min")
...
rank.order.lists = list(
  Iso = c("NA", "100pM", "300pM", "1nM", "3nM", "10nM", "30nM", "100nM", "300nM"),
  NECA = c("NA", "30nM", "100nM", "300nM", "1μM", "3μM", "10μM", "30μM", "100μM"),
  VIP = c("NA", "10pM", "30pM", "100pM", "300pM", "1nM", "3nM", "10nM", "30nM")
)

# ISO: series.rank.order = c("NA", "100pM", "300pM", "1nM", "3nM", "10nM", "30nM", "100nM", "300nM")
# NECA: series.rank.order = c("NA", "30nM", "100nM", "300nM", "1μM", "3μM", "10μM", "30μM", "100μM")
# VIP: series.rank.order = c("NA", "10pM", "30pM", "100pM", "300pM", "1nM", "3nM", "10nM", "30nM")
...
# First, split SUBJECT into 4 columns, cell.line, agonist.conc, timepoint, and replicate #
# this makes splitting easier
experiment.proteinData[, SUBJECT := gsub("\\.", "_", SUBJECT)]

experiment.proteinData <- experiment.proteinData %>%
  separate(SUBJECT, into = c("cell.line", "agonist.conc", "timepoint", "SUBJECT"), sep = "_", remove = TRUE) %>%
    mutate(label = paste(`cell.line`, `timepoint`, sep = '_'))
...
PROTEIN.data <- lapply(series.labels, function(series.label) {
  ...
  vehicle.term = paste(unlist(strsplit(series.label, "_"))[1], "vehicle", sep = "_")
  
  subset <- experiment.proteinData %>%
  filter(grepl(series.label, label) | label == vehicle.term)
...
PROTEIN.data <- lapply(series.labels, function(series.label) {
  ...
  rank.order.label <- series.subset$cell.line[1]
  
  series.rank.order <- rank.order.lists[[rank.order.label]]
  ...
  series.subset <- series.subset %>%
    mutate(rankTime = -1) %>%
    mutate(rankTime = match(agonist.conc, series.rank.order) - 1)
...
invisible(lapply(polyfit.names, function(polyfit.name) {
  ...
  rank.order.label <- str_split(polyfit.name, "_")[[1]][1]
  ...
  lapply(target.proteins, function(protein) {
    ...
    protein.specific.data[, agonist.conc := factor(agonist.conc, levels = series.rank.order)]
    ...
    regression.prediction <- melt(
      ...
      variable.name = "agonist.conc", 
      value.name = "Prediction Abundance"
    )
    regression.prediction <- regression.prediction[, c("agonist.conc", "Prediction Abundance")]
    regression.prediction$agonist.conc <- series.rank.order
    regression.prediction$`Prediction Abundance` <- regression.prediction$`Prediction Abundance` - vehicle.abundance
    regression.prediction[, agonist.conc := factor(agonist.conc, levels = series.rank.order)]
    regression.prediction$color <- "#323336"
    
    # Render plot.
    p <- ggplot(protein.specific.data, aes(x = agonist.conc, y = polyfit.intensity, color = SUBJECT)) +
      ...
      geom_path(
        ...
        group = 1, 
        aes(x = agonist.conc, y = `Prediction Abundance`),
        linetype = "dashed",
        color = "#323336",
        linewidth = 1, 
      ) +
    ...
    ggsave(paste0(paste(paste(data.name, "figures", sep = "_"), "Regressions", polyfit.name, gene.name, sep = "/"), " - Anova Regression.pdf"), plot = p, create.dir = TRUE)
...
invisible(lapply(polyfit.names, function(polyfit.name) {
  ...
  ggplot(thisDataset, aes(x = log2FC, y = -log10(pvalue), color = Significance, label = gene.name)) +
    ...
    labs(title = paste("EB10", polyfit.name, "Abundance Concentration ANOVA", sep = " - "), x = paste("Protein - Maximum log2FC"), y = "-log10(p-value)") +
...
# Heatmap with Concentration-level data without labels
...
column_clusters <- rep(1:3, each = 18)

pdf(paste(paste(data.name, "figures", sep = "_"), "Static HeatMap - Concentration Series ANOVA.pdf", sep = "/"))

heatmap <- Heatmap(
  Sig.Changing.Site.Matrix,
  column_title = "EB10 - Abundance - Concentration Series ANOVA",
  row_title = sprintf("%d Significantly Changing Proteins vs Vehicle", nrow(Sig.Changing.Site.Matrix)),
  name = "Log2FC From Vehicle\n",
  show_row_names = TRUE,
...
pdf(paste(paste(data.name, "figures", sep = "_"), "HeatMap - Concentration Series ANOVA.pdf", sep = "/"))
